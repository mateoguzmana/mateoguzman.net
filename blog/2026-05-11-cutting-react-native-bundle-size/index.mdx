...
- `IS_QA_TOOLS_ENABLED` comes from `react-native-config`, which reads `.env` at runtime through the native side. Metro can't statically resolve it at bundle time, so the conditional looks dynamic to the bundler, the branch stays reachable, and the `import { Storybook }` at the top of `AppRouter.tsx` is a live edge in the module graph.
...
<figure>
  <svg viewBox="0 0 640 240" role="img" aria-label="Yarn resolutions: two near-identical versions of an external SDK family in the bundle collapse into one.">
    ...
    <text x="40" y="72" fill="currentColor" fontSize="11" fontWeight="600">@wallet-provider/*</text>
    ...
    <text x="159" y="72" fill="currentColor" fontSize="11" fontWeight="600">@wallet-provider/*</text>
    ...
    <text x="380" y="72" fill="currentColor" fontSize="11" fontWeight="600">@wallet-provider/*</text>
...
On the treemap, one external SDK family was the obvious shape: `@wallet-provider/utils`, `@wallet-provider/core`, `@wallet-provider/sign-client`, `@wallet-provider/types`, `@wallet-provider/universal-provider` all showed up *twice each*, at two slightly different version numbers, with near-identical sizes. Same story for a chain SDK (`@some-chain/web3-sdk`). A `yarn why` confirmed it:

```sh
$ yarn why @wallet-provider/utils
# → @wallet-provider/utils@npm:2.21.x   (hoisted, used directly)
# → @wallet-provider/utils@npm:2.17.x   (pulled in by another upstream SDK)
```
...
// package.json
"resolutions": {
  "@some-chain/web3-sdk": "1.98.4",
  "@wallet-provider/core": "2.23.9",
  "@wallet-provider/sign-client": "2.23.9",
  "@wallet-provider/types": "2.23.9",
  "@wallet-provider/universal-provider": "2.23.9",
  "@wallet-provider/utils": "2.23.9"
}
```
...
That smoke test isn't optional. I picked the most-trafficked screen for each pinned package family — the actual wallet-provider flow for the `@wallet-provider/*` packages, the actual chain transaction path for `@some-chain/web3-sdk` — and ran the full happy path through QA on both platforms before merging. Pin alone, no smoke, no merge.
...
One subtle thing that bit me before it stopped biting me: the WebView needs to fully remount when the theme changes, not just receive a new `LOAD_CONFIG`. Otherwise the chart library renders some styles before `LOAD_CONFIG` arrives, and you end up with a half-themed first paint. Keying the element on `mode` (`key={`chart-webview-${mode}`}`) is the cheapest way to force the remount. React unmounts the old WebView, the new one starts fresh, and the very first paint reads the right theme.
